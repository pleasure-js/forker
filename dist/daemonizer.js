 ...
 */
class RunningProcess extends events.EventEmitter {
  /**
  ...
  start () {
    ...
    this._spawnChild = child_process.spawn(this._spawnArgs.command,
...
schemaValidator.Transformers.id = {
  settings: {
    loaders: [{
      type: String,
 ...
 * @type {Schema}
 */
const RequestSchema = new schemaValidator.Schema({
  id: {
    type: 'id',
    required: false
  },
...
class DaemonizerServer extends events.EventEmitter {
  constructor (config = {}) {
    super();
    ...
    // save pid
    fsExtra.writeJsonSync(this.config.runningThread, Object.assign({}, this.config, { pid: process.pid }));
  ...
  static isRunning (config = {}) {
    ...
    if (fs.existsSync(config.runningThread)) {
      const { pid } = fsExtra.readJsonSync(config.runningThread);
      if (isRunning(pid)) {
        return pid
      }
    }
  ...
  static start (config = {}, env = {}) {
    ...
    const child = child_process.spawn(`node`, [__filename, `&>${ path.join(__dirname, '../daemon.log') }`], {
      detached: true,
      stdio: 'ignore',
...
exports.DaemonizerServer = DaemonizerServer;
